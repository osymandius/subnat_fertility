  ...
  
{
  ...
  nll -= dnorm(u_spatial_str, Type(0), Type(1), true).sum();
  nll -= dnorm(u_spatial_str.sum(), Type(0), Type(0.01) * u_spatial_str.size(), 1); // sum to zero constraint
  ...
  vector<Type> log_lambda(X_mf * beta_mf
                     ...
                     + Z_spatial * spatial 
                     + Z_interaction1 * eta1_v * tau_eta1
