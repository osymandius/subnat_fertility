...
asfr <- get_asfr_pred_df("ZWE", 0, project = FALSE)
...
R_spatial0 <- .sparseDiagonal(nrow(R_spatial), shape = "t")
R_period0 <- .sparseDiagonal(nrow(R_period), shape = "t")
R_age0 <- .sparseDiagonal(nrow(R_age), shape = "t")
...
mf$mf_model %>%
  ...
  type.convert() %>%
  bind_rows(inla_res %>% mutate(source = "inla")) %>%
  ggplot(aes(x=period, y=median, ymin = lower, ymax = upper,
