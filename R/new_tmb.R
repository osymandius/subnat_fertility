...
if(mf$mics_toggle) {
  ...
  tmb_int$par <- c(tmb_int$par, "u_tips_mics" = rep(0, ncol(Z_tips_mics)))
...
# mf$mf_model %>%
  ...
  geom_line() +
  geom_point(data=asfr_plot %>% left_join(areas_long) %>% filter(area_level == 1), aes(y=asfr, group=survtype, color=survtype)) +
