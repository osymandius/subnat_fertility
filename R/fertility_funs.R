...
area_populations <- function(population, areas_wide) {
  ...
  base_area_pop <- left_join(areas_wide, population, by = c("area_id", "iso3"))
...
make_model_frames <- function(iso3_current, population, asfr, mics_asfr,
  ...
  # ) %>%
  #   left_join(population %>% filter(period == min(period)) %>% dplyr::select(-period)) %>%
