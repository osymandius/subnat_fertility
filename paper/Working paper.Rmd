...
Fertility estimation is core to understanding evolving demographic trends and population structures in sub-Saharan Africa. In high-income settings, long-standing vital registration systems provide direct measures of fertility, but these are often underdeveloped or absent in low and middle income countries. Instead, the reconstruction of fertility trends from survey- and census-based surveillance sources in these settings is well established, by which quinqunnial national-level fertility estimates for sub-Saharan Africa are published by UN Population Division for the period 1970-2050 as part of the World Population Prospects (WPP). National population estimates, underpinned by these reconstructed trends in concert with estimates of mortality and migration, are utilised throughout domestic policymaking. 

While quinquennial national-level estimates of total fertility are highly desirable for policymaking with long projection horizons - e.g. future demand for schooling or designing large scale infrastructure projects - they are less well suited for use in public health target setting and monitoring which often require higher resolution estimates, both temporally and spatially. 

Accordingly, public health policy makers and funders are increasingly interested in subnational population and demographic estimates furnished at the level of local decision making – often the second administrative level - which enable targeted and efficient resource allocation. Whilst mortality estimates, particularly those of under 5 mortality, have received more attention at finer geospatial disaggregation, there are fewer sources of subnational fertility estimates.

- FP, PMTCT
- MDG/SDG on SRH

Estimates of total fertility rate at the first administrative level are produced in X countries with the support of the US Census Bureau as part of demographic inputs into the Spectrum modelling tools, though these are derived from fixed national-level age-specific fertility rates. The WorldPop project produce 1km x 1km pixel-level estimates of births, using suvery-derived age-specific fertility rates at the first administrative level applied to pixel-level age-specific populations [@Tatem2014]. At present there exist no age-specific fertility estimates for district-level decision-making estimated from granular data at that level [do check this is true...]. 
